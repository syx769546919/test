    ...
    <%@ page language="java" contentType="text/html; charset=utf-8" import="java.util.*,com.iotek.entity.*"
    pageEncoding="utf-8"%>
   			...
   			$.ajax({
   				...
   				success:function(data){
   					...
   					$.each(data.cartList, function(index,item){
   						var str = 
   							'<li>' + 
			                	'<div class="img"><a href="#"><img src="' + item.goods.picture + '" width="58" height="58" /></a></div>' +
			                    '<div class="name"><a href="#">' + item.goods.name + '</a></div>' +
...
<div class="soubg">
	<div class="sou">
        ...
        <!--End 所在收货地区 End-->
        <span class="fr">
        	<span class="fl">欢迎 ${sessionScope.user.userName}&nbsp;&nbsp; <a href="Regist.html" style="color:#ff4e00;">免费注册</a>&nbsp;|&nbsp;<a href="#">我的订单</a>&nbsp;|</span>
        	<span class="ss">
            	<div class="ss_list">
                	<a href="#">收藏夹</a>
...
<div class="top">
    ...
    <div class="search">
    	<form action="search.do" method="post">
        	<input type="text" value="${requestScope.key}" class="s_ipt" name="key" />
            <input type="submit" value="搜索" class="s_btn" />
        </form>                      
        <span class="fl"><a href="#">咖啡</a><a href="#">iphone 6S</a><a href="#">新鲜美食</a><a href="#">蛋糕</a><a href="#">日用品</a><a href="#">连衣裙</a></span>
    </div>
    <div class="i_car">
    	<div class="car_t">购物车 [ <span id="cartCount">${cartInfo.totalCount}</span> ]</div>
        <div class="car_bg">
       		<!--Begin 购物车未登录 Begin-->
        	<div class="un_login"><a > 查看购物车</a></div>
            <!--End 购物车未登录 End-->
            <!--Begin 购物车已登录 Begin-->
             <ul class="cars" id="cartList">
            	<c:forEach items="${cartInfo.cartList}" var="sc">
            		<li>
	                	<div class="img"><a href="#"><img src="${sc.goods.picture}" width="58" height="58" /></a></div>
	                    <div class="name"><a href="#">${sc.goods.name}</a></div>
	                    <div class="price"><font color="#ff4e00">
	                    	￥<fmt:formatNumber value="${sc.goods.prize}" pattern="#,##0.00"></fmt:formatNumber> 
	                    	</font>X ${sc.qty}</div>
	                </li>
            	</c:forEach>
            </ul>
            <div class="price_sum">共计&nbsp; <font color="#ff4e00">￥</font>
            	<span id="totalAmount"><fmt:formatNumber value="${cartInfo.amount}" pattern="#,##0.00"></fmt:formatNumber></span></div>
            <div class="price_a"><a href="shopcar.jsp">去购物车结算</a></div>
            <!--End 购物车已登录 End-->
        </div>
    </div>
</div>
    ...
    <div class="content mar_20">
        ...
        <div class="l_list">
            ...
            <div class="list_c">
                ...
                <ul class="cate_list">
                <%-- <% 
                	Object obj = request.getAttribute("pagination");
                	if(obj!=null){
                		Pagination p=(Pagination)obj;
                		ArrayList<Goods> productList = p.getDatalist();
                		for(Goods product:productList){
                %>
                	<li>
                    	<div class="img"><a href="#"><img src="<%=product.getPicture() %>" width="210" height="185" /></a></div>
                        <div class="price">
                            <font>￥<span><%=product.getPrize() %></span></font> &nbsp; 26R
                        </div>
                   ...
                   %>  --%> 
                   <c:forEach items="${pagination.datalist}" var="goods">
                		<li>
	                    	<div class="img"><a href="#"><img src="${goods.picture}" width="210" height="185" /></a></div>
	                        <div class="price">
	                            <font>￥<span>${goods.prize}</span></font>
	                        </div>
	                        <div class="name"><a href="#">${goods.name}</a></div>
	                        <div class="carbg">
	                        	<a href="#" class="ss">收藏</a>
	                            <a href="javascript:addCart(${goods.id})" class="j_car">加入购物车</a>
	                        </div>
	                    </li>
                	</c:forEach>
                </ul>
                ...
                <div class="pages">
                <c:if test="${pagination.currpage eq 1}">
					<a href="#" class="p_pre">首页</a><a href="#" class="p_pre">上一页</a>
				</c:if>
                
                <c:if test="${pagination.currpage gt 1}">
                <a href="search.do?currPageStr=1&key=${requestScope.key}" class="p_pre">首页</a>
                <a href="search.do?currPageStr=${pagination.currpage-1}&key=${requestScope.key}" class="p_pre">上一页</a>
                </c:if>
                <c:forEach begin="1" end="${pagination.totalpage}" step="1" var="page">
                	<c:if test="${pagination.currpage eq page}">
                		<a href="#" class="cur">${page}</a>
                	</c:if>
                	<c:if test="${not (pagination.currpage eq page)}">
                		<a href="search.do?currPageStr=${page}&key=${requestScope.key}">${page}</a>
                	</c:if>
                </c:forEach>
                <c:if test="${pagination.totalpage eq pagination.currpage }">
					<a href="#" class="p_pre">下一页</a><a href="#" class="p_pre">尾页</a>
				</c:if>
                <c:if test="${not (pagination.totalpage eq pagination.currpage) }">
               	 <a href="search.do?currPageStr=${pagination.currpage+1}&key=${requestScope.key}" class="p_pre">下一页</a>
                <a href="search.do?currPageStr=${pagination.totalpage}&key=${requestScope.key}" class="p_pre" >尾页</a>
                </c:if>
